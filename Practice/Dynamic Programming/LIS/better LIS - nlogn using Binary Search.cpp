...
// O(NlogN) solution using certain optimisation
...
{   ios_base::sync_with_stdio(false);
    ...
    //progress.
    for (int i = 0; i < n; i++) {
        //check for the len of dp's where it can be allocated.strictly increasing.
        //optimisation
        int len = upper_bound(dp, dp + n + 1, arr[i]) - dp;
        if (dp[len - 1] < arr[i] and arr[i] < dp[len]) //this is the right place.
            dp[len] = arr[i];
    }
